...
/* App constants */
const unsigned long blinkHz = 1000 / 10;
...
enum MyMcuAnnuciatorValues
{
    ...
    MYMCU_ANNUC_CAUT_BLINK  = 0x3,
    MYMCU_ANNUC_WARN_BLINK  = 0x5
};
...
boolean blinkCaution = false;
boolean blinkWarning = false;
unsigned long cautionBlinkTimer, warningBlinkTimer;

void processAnnunciator(byte value)
{
    switch(value)
    {
    case MYMCU_ANNUC_CAUT_BLINK:
        blinkCaution = true;
        cautionBlinkTimer = millis();
...
void loop()
{
    ...
    */
    ...
    /*
    * Blink warning/caution if desired
    */
    if (blinkCaution && msNow - cautionBlinkTimer > blinkHz)
    {
        digitalWrite(cautionLedPin, digitalRead(cautionLedPin) == HIGH ? LOW : HIGH);
        cautionBlinkTimer = msNow;
    }
    if (blinkWarning && msNow - warningBlinkTimer > blinkHz)
    {
        digitalWrite(warningLedPin, digitalRead(warningLedPin) == HIGH ? LOW : HIGH);
        warningBlinkTimer = msNow;
    }
